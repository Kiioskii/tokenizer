...
def run_test(text):
    ...
    gpt_ids = enc.encode(text, allowed_special="all")
    gpt_decoded = enc.decode(gpt_ids)

    tokenizer = build_tokenizer_from_gpt4(enc)

    my_ids = tokenizer.encode(text, allowed_special="all")
...
if __name__ == "__main__":
    run_test("<|endoftext|>hello world!!!? (안녕하세요!) lol123 😉")
